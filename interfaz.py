...
class CodePadMainWindow(tk.Frame):
    ...
    def __init__(self, root, *args, **kwargs):
        ...
        self.buildMenubar()
        # self.toolbar = self.buildToolbar(self)
        self.console = self.buildConsole(self)
    ...
    def buildConsole(self, parent, *args, **kwargs):
        self.console = tk.Text(parent, height=10)
        self.console_title = tk.Frame(parent, bd=1, relief=tk.RAISED,height=200)
        self.title = tk.StringVar(self.console_title)
        self.title.set('Console')
        statusLabel = tk.Label(self.console_title, textvariable=self.title, relief=tk.SUNKEN)
        statusLabel.grid(row=0, column=0, sticky="EW")
    ...
    def runCurrent(self):
        ...
        while(start_index >= 0):
            ...
            end_index = current_code.find('}', start_index)
            ...
            start_index = current_code.find(') {', end_index)
...
class CodePad(tk.Tk):
    TITLESTRING = 'Kotlin Awesome Code Editor'
